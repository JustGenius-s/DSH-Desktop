...
const { dshAuthCookieUrl, isDshAuthCookie } = require('../dist/main/platform/auth-cookies.js')
...
test('builds a removable URL for each cookie variant', () => {
  ...
  assert.equal(
    dshAuthCookieUrl({ domain: '.127.0.0.1', path: '/nested' }),
    'http://127.0.0.1/nested',
  )
  assert.equal(
    dshAuthCookieUrl({ domain: '127.0.0.1', path: '/secure', secure: true }),
