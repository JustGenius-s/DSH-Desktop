...
import { Ipc } from './ipc'
import { installedDshVersion, latestDshAcrossChannels, updateDsh } from './runtime-manager'
import { clearDshUpdate, setUpdateResult } from './update-state'

/** 兜底下载页：拿不到具体 release URL 时用它。 */
export const RELEASES_FALLBACK = 'https://github.com/JustGenius-s/DSH-Desktop/releases/latest'
...
/** 菜单刷新钩子：由 setupDesktopBridge 的调用方注入（避免与 desktop-seats 循环依赖）。 */
...
/** 查一轮：App 本体 + DSH 运行时。网络失败静默，保持上次结果。 */
...
export async function updateDshRuntime(version?: unknown): Promise<void> {
  ...
  const target = typeof version === 'string' && version !== '' ? version : await latestDshAcrossChannels()
...
export function setupDesktopBridge(): void {
  ...
  ipcMain.handle(Ipc.updates.downloadApp, () => {
    void shell.openExternal(RELEASES_FALLBACK)
  })
